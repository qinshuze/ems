				...
				<div class="box itembox" style="padding-top:10px;margin-bottom:0px;">
			        ...
			        <form action="?exam-master-basic-setexamrange" method="post" class="form-horizontal">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<label class="radio-inline">
						          	<input type="radio" name="args[basicexam][model]" value="0"{x2;if:$basic['basicexam']['model'] == 0} checked{x2;endif}/> 全功能模式（显示练习，显示试题库）
					          	</label>
					          	<label class="radio-inline">
					          		<input type="radio" name="args[basicexam][model]" value="1"{x2;if:$basic['basicexam']['model'] == 1} checked{x2;endif}/> 练习模式（显示练习，不显示试题库）
					          	</label>
					          	<label class="radio-inline">
					          		<input type="radio" name="args[basicexam][model]" value="2"{x2;if:$basic['basicexam']['model'] == 2} checked{x2;endif}/> 考试模式（仅显示正式考试）
					          	</label>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-9">
								<label class="radio-inline">
						          	<input type="radio" name="args[basicexam][changesequence]" value="0"{x2;if:$basic['basicexam']['changesequence'] == 0} checked{x2;endif}/> 不打乱试题排序
					          	</label>
					          	<label class="radio-inline">
					          		<input type="radio" name="args[basicexam][changesequence]" value="1"{x2;if:$basic['basicexam']['changesequence'] == 1} checked{x2;endif}/> 打乱试题排序
					          	</label>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-9 form-inline">
								<input class="form-control" name="args[basicexam][batch]" value="{x2;$basic['basicexam']['batch']}"/>
								<span class="help-block">用户考试次数以批次为单位，可用于相同用户群体多次考试。</span>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<label class="radio-inline">
					          		<input type="radio" name="args[basicexam][selectrule]" value="1"{x2;if:$basic['basicexam']['selectrule']} checked{x2;endif}/> 系统随机抽卷
					          	</label>
					          	<label class="radio-inline">
					          		<input type="radio" name="args[basicexam][selectrule]" value="0"{x2;if:!$basic['basicexam']['selectrule']} checked{x2;endif}/> 用户手动选卷
					          	</label>
								<span class="help-block">系统随机抽卷时，用户无法看到试卷列表，系统会随机挑选试卷供用户考试。手选试卷时，系统列出试卷列表供用户选择进行考试。</span>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-sm-3">未答完题禁止交卷：</label>
							<div class="col-sm-9">
								<label class="radio-inline">
					          		<input type="radio" name="args[basicexam][fullsubmit]" value="1"{x2;if:$basic['basicexam']['fullsubmit']} checked{x2;endif}/> 开启
					          	</label>
					          	<label class="radio-inline">
					          		<input type="radio" name="args[basicexam][fullsubmit]" value="0"{x2;if:!$basic['basicexam']['fullsubmit']} checked{x2;endif}/> 关闭
					          	</label>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-sm-3">考试开始后：</label>
							<div class="col-sm-9 form-inline">
								<input class="form-control" name="args[basicexam][unjointime]" value="{x2;$basic['basicexam']['unjointime']}"/>
					          	<label class="radio-inline">
					          		分钟禁止抽卷
					          	</label>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-9 form-inline">
								...
								<span class="help-block">不限制次数请填写0；如果设置限考次数为x，当随机抽卷时，用户每批次一共可以考试x次；手选试卷时，用户每批次每套试卷可考试x次。</span>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-9">
								<label class="radio-inline">
						          		<input type="radio" name="args[basicexam][notviewscore]" value="0"{x2;if:$basic['basicexam']['notviewscore'] == 0} checked{x2;endif}/> 直接显示分数
					          		</label>
					          		<label class="radio-inline">
					          			<input type="radio" name="args[basicexam][notviewscore]" value="1"{x2;if:$basic['basicexam']['notviewscore'] == 1} checked{x2;endif}/> 暂不显示分数
					          		</label>
							</div>
						</div>
						<div class="form-group">
