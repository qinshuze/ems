			        ...
			        <form action="index.php?exam-master-exams-autopage" method="post" class="form-horizontal">
						...
						<div class="form-group">
					        <label class="control-label col-sm-2">题量配比模式：</label>
				          	<div class="col-sm-9">
								<label class="radio-inline">
					          		<input type="radio" class="form-control" name="args[examsetting][scalemodel]" value="1" onchange="javascript:changemode($(this).val())"/> 开启
					          	</label>
					          	<label class="radio-inline">
					          		<input type="radio" class="form-control" name="args[examsetting][scalemodel]" value="0" onchange="javascript:changemode($(this).val())" checked/> 关闭
					          	</label>
					       </div>
					    </div>
					    <div id="modeplane"></div>
						<div class="form-group">
					...
					<div id="sptype" class="hide">
						...
						<div class="form-group questpanel panel_{x2;v:questype['questid']}">
							...
							<div class="col-sm-9 form-inline">
								...
								<a class="btn btn-primary ajax" target="tablecontent" href="index.php?exam-master-exams-ajax-getsubjectknows&questype={x2;v:questype['questid']}&subjectid={examsubjectid}" valuefrom="examsubjectid" onclick="javascript:currentP = 'examscale_{x2;v:questype['questid']}';$('#tablecontent').find(':checkbox').attr('checked',false);$('#modal').modal();">配题</a>
...
{
	$.getJSON('index.php?exam-master-basic-getsubjectquestype&subjectid='+$(obj).val()+'&'+Math.random(),function(data){
		$('.questpanel').hide();
		$('.questypepanelinput').val('0');
		for(x in data){
			$('.panel_'+data[x]).show();$('#panel_'+data[x]).val('1');
		}
	});
}
...
function changemode(type)
{
	if(type == 1)
	{
		$('#modeplane').children().appendTo($('#normaltype'));
		$('#sptype').children().appendTo($('#modeplane'));
	}
	else
	{
		$('#modeplane').children().appendTo($('#sptype'));
		$('#normaltype').children().appendTo($('#modeplane'));
	}
}
changemode(0);
</script>
