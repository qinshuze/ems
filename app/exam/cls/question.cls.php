...
class question_exam
{
			...
			foreach($s1 as $s2)
			{
				...
				if(is_array($num))
				{
					...
					foreach($number as $nkey => $t)
					{
						...
						{
							if($expqr)
							$qrs = $this->getRandQuestionRowsList(array(array("AND","quest2knows.qkknowsid in (:knowsids)",'knowsids',$knowsids),array("AND","questionrows.qrid NOT IN (:expqr)",'expqr',$expqr),array("AND","questionrows.qrlevel = :qrlevel",'qrlevel',$nkey),array("AND","questionrows.qrtype = :qrtype",'qrtype',$key),array("AND","questionrows.qrnumber <= :qrnumber",'qrnumber',$t),array("AND","questionrows.qrnumber > 0")));
							else
				...
				else
				{
					...
					{
						if($expqr)
						$qrs = $this->getRandQuestionRowsList(array(array("AND","quest2knows.qkknowsid in (:knowsids)",array("AND","questionrows.qrtype = :qrtype",'qrtype',$key),array("AND","questionrows.qrid NOT IN (:expqr)",'expqr',$expqr),array("AND","questionrows.qrnumber <= :qrnumber",'qrnumber',$t))));
						else
	...
	public function selectQuestions($examid,$basic)
	{
		...
		foreach($settings['questype'] as $key => $p)
		{
			...
			foreach($number as $nkey => $t)
			{
				...
				{
					if($expqr)
					$qrs = $this->getRandQuestionRowsList(array(array("AND","quest2knows.qkknowsid in (:knowsids)",'knowsids',$knowsids),array("AND","questionrows.qrlevel = :qrlevel",'qrlevel',$nkey),array("AND","questionrows.qrtype = :qrtype",'qrtype',$key),array("AND","questionrows.qrid NOT IN (:expqr)",'expqr',$expqr),array("AND","questionrows.qrnumber <= :qrnumber",'qrnumber',$t)));
					else
					$qrs = $this->getRandQuestionRowsList(array(array("AND","quest2knows.qkknowsid in (:knowsids)",'knowsids',$knowsids),array("AND","questionrows.qrlevel = :qrlevel",'qrlevel',$nkey),array("AND","questionrows.qrtype = :qrtype",'qrtype',$key),array("AND","questionrows.qrnumber <= :qrnumber",'qrnumber',$t)));
					if(count($qrs))
					{
	...
	{
		$data = array(false,'questions',array(array('AND',"questionid = :questionid",'questionid',$questionid)));
		$sql = $this->pdosql->makeSelect($data);
